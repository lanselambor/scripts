...
def readInt16(buf):
  return ord(buf[0:1])*256 + ord(buf[1:2])
...
class Connects(Packets):
  ...
  def unpack(self, buffer):
    ...
    try:
      ...
      cur_next = curlen + 1
      self.ProtocolVersion = ord(buffer[curlen:cur_next])
      curlen += 1
      cur_next = curlen + 1

      connectFlags = ord(buffer[curlen:cur_next])
...
class Connacks(Packets):
  ...
  def unpack(self, buffer):
    ...
    assert ord(buffer[2:3]) in  [0, 1], "Connect Acknowledge Flags"
    self.returnCode = ord(buffer[3:4])
    assert self.fh.DUP == False, "[MQTT-2.1.2-1]"
